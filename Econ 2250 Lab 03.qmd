...
Hello Thomas Cochran wrote this
...
#| # Heatmap of cps numeric correlations
library(tidyverse)

num_vars <- c("wage", "hrslastwk", "educ", "earnwk")

cps_num <- cps |>
  select(any_of(num_vars)) |>
  mutate(across(everything(), as.numeric))

corr_mat <- cor(cps_num, use = "pairwise.complete.obs")
corr_df  <- as.data.frame(as.table(corr_mat)) |>
  rename(var_x = Var1, var_y = Var2, r = Freq)

ggplot(corr_df, aes(var_x, var_y, fill = r)) +
  ...
  labs(title = "cps: Correlation Heatmap",
...
# Radar chart: mean weekly earnings, hours, and education by gender
library(tidyverse)
library(fmsb)
library(scales)

sex_col   <- "gender"      # Male / Female
wage_col  <- "earnwk"      # weekly earnings
hours_col <- "hrslastwk"   # hours worked last week
educ_col  <- "educ"        # years of education
...
sum_tab <- cps |>
  ...
  summarise(
    .by = .data[[sex_col]],
    Weekly_Earnings = mean(.data[[wage_col]],  na.rm = TRUE),
    Hours_Last_Week = mean(.data[[hours_col]], na.rm = TRUE),
    Education_Years = mean(.data[[educ_col]],  na.rm = TRUE)
  ) |>
  rename(Group = 1) |>
...
# ---- rescale 0–100
scaled <- sum_tab |>
  mutate(across(everything(), ~ rescale(.x, to = c(0, 100)))) |>
  as.data.frame()

# ---- fmsb requires first 2 rows = max/min with IDENTICAL column names
bounds <- data.frame(
  matrix(c(rep(100, ncol(scaled)), rep(0, ncol(scaled))),
         nrow = 2, byrow = TRUE)
)
colnames(bounds) <- colnames(scaled)
rownames(bounds) <- c("max", "min")

rad_dat <- rbind(bounds, scaled)

# ---- plot
cols <- scales::alpha(hcl(h = seq(15, 375, length.out = nrow(scaled)+1)[1:nrow(scaled)],
                          c = 70, l = 60), 0.35)
...
radarchart(
  ...
  title = "cps: Radar of Mean Weekly Earnings, Hours, Education by Gender"
)
legend("topright", legend = rownames(scaled), bty = "n", cex = 0.85, fill = cols)
par(op)
